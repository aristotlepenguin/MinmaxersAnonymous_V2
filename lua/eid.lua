...
if EID then
    ...
    EID:addCollectible(mod.MMATypes.COLLECTIBLE_D_SQRT, "Меняет все предметы в комнате. Однако для расчета используется очень специфическая формула.#Формула следующая: (<Кол-во бомб> * <старый номер предмета>^2 + <кол-во монет> * <старый номер предмета> + секунды на таймере + 109 )/9 % <общее количество предметов, включая модифицированные>.#Если конечный номер предмета не является натуральным числом, он превратиться в \"Какашку\".", "D-Квадрат(-1)", "ru")
